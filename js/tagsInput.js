...
function doIt(persadi) {
    myArr = [];
    lengthTag = myArr.length;
    $('.txtSearchProdAssign#' + persadi).parent().append('<br><div id="apentest-' + persadi + '" class="form-group" style="height:37px;overflow:auto;margin:0 auto;border:0px solid #d0d0d0;border-radius:3px;"></div>');
    var nameTag = $('.txtSearchProdAssign#' + persadi);
    var idTag = $('.txtSearchProdAssign#' + persadi).attr('id');
    $('.txtSearchProdAssign#' + persadi).parent().append('<input type="text" name="' + nameTag.attr('name') + '" id="' + nameTag.attr('name') + '" style="display: none;">');
    $('.txtSearchProdAssign#' + persadi).attr('class', 'form-control txtSearchProdAssign-' + persadi);
    nameTag.removeAttr('name');
    nameTag.removeAttr('id');

    setValue = function (persadi) {
        if ((persadi)) {
            var myArr = [];
        }
    ...
    automateDisable = function (persadi) {
        var points = $('#apentest-' + persadi).find('button').length;
        var attempts = $('input[name=sr_attempts]').val();
    ...
    createArr = function (valNilai, persadi) {
        if (valNilai) {
    ...
    tagDisable = function () {
        $('[id^=apentest] :button').attr('disabled', true);
        if (automateDisable(persadi) === true) {
            $('.addon-sm').find('input').attr('disabled', true);
        }
    }

    tagEnable = function () {
        $('[id^=apentest] :button').attr('disabled', false);
        if (automateDisable(persadi) === false) {
            $('.addon-sm').find('input').attr('disabled', false);
        }
    }
}
(function ($) {
